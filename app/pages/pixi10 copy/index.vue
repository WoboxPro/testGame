...
onMounted(async () => {
  if (process.client && pixiContainer.value) {
    ...
    const graphics = createPlayer(400, 300);
    app.stage.addChild(graphics);

    const graphics2 = createObstacle(200, 250, 1);
    app.stage.addChild(graphics2);

    // --- Система коллизий ---
    const obstacles = [graphics2];
    const respawnDelay = 2000;
    const respawn = createRespawnFunction(respawnDelay, app.screen);
    ...
    const effectContext = {
      ...
      explosions,
      obstacles,
      respawnCallback: respawn
    };
    
    function localTriggerBulletEvent(bullet, eventName, ticker, extraData = {}) {
      triggerBulletEvent(
        bullet, 
        eventName, 
        ticker, 
        weaponConfig, 
        (bullet, effectName, extraData) => applyStandardEffect(bullet, effectName, effectContext, extraData),
        extraData
      );
    }
    ...
    function createRaycast() {
      ...
      const ammoState = { currentAmmo, isReloading };
      if (!canFireWeapon(currentTime, lastFireTime, weaponConfig, ammoState)) return;
      ...
      for (let i = 0; i < weaponConfig.bulletsPerShot; i++) {
        ...
        const finalAngle = calculateFinalAngle(baseAngle, i, weaponConfig);
        
        // Вычисляем индивидуальную дальность с разбросом для луча
        const rayMaxRange = calculateRangeWithSpread(weaponConfig);
      ...
      createMuzzleFlash(graphics.x, graphics.y, app, impactEffects);
      
      // --- Расходуем патроны ---
      const ammoStateForConsume = { currentAmmo, isReloading };
      consumeAmmo(ammoStateForConsume, weaponConfig, weaponConfig.bulletsPerShot);
      currentAmmo = ammoStateForConsume.currentAmmo; // Обновляем состояние
      
      // --- Применяем отдачу оружия ---
      applyRecoil(graphics, baseAngle, weaponConfig, app.screen);
    ...
    function performRaycast(startX, startY, angle, maxRange, penetrationLeft, ricochetsLeft, hasTriggeredScreenEdge = false) {
      ...
      while (travelDistance < maxRange && penetrationLeft > 0) {
        ...
        if (isAtScreenEdge && !hasTriggeredScreenEdge) {
          ...
          localTriggerBulletEvent(fakeRayBullet, 'onScreenEdge', fakeTicker);
        ...
        if (weaponConfig.ricochetWalls && ricochetsLeft > 0) {
          const ricochetResult = calculateRaycastWallRicochet(currentX, currentY, angle, app.screen);
          
          if (ricochetResult.hasRicocheted) {
            currentX = ricochetResult.x;
            currentY = ricochetResult.y;
            // --- Событие: onRicochet (для raycast от стен) ---
            const fakeRayBullet = { x: currentX, y: currentY };
            const fakeTicker = { elapsedMS: 0 };
            localTriggerBulletEvent(fakeRayBullet, 'onRicochet', fakeTicker);
            
            // Создаем визуальный эффект до точки рикошета
            if (weaponConfig.raycastAnimation === 'laser') {
              createRayVisual(startX, startY, currentX, currentY, app, rayEffects);
            } else if (weaponConfig.raycastAnimation === 'impact') {
              createImpactPoint(currentX, currentY, 'ricochet', app, impactEffects);
            }
            
            // Продолжаем луч после рикошета
            ricochetsLeft--;
            return performRaycast(currentX, currentY, ricochetResult.angle, maxRange - travelDistance, penetrationLeft, ricochetsLeft, hasTriggeredScreenEdge);
          }
        }
        
        // Проверяем выход за границы экрана (если рикошеты кончились)
        if (checkScreenBounds({ x: currentX, y: currentY }, app.screen, weaponConfig)) {
          break;
        }
        ...
        for (const obstacle of obstacles) {
          if (obstacle.isAlive && !hitTargets.includes(obstacle)) {
            ...
            if (distance <= obstacleRadius) {
              ...
              dealDamage(obstacle, weaponConfig.bulletDamage, respawn);
              
              // --- Событие: onHitEnemy (для raycast) ---
              const fakeRayBullet = { x: currentX, y: currentY };
              // Для raycast нет ticker, создаем фейковый
              const fakeTicker = { elapsedMS: 0 };
              localTriggerBulletEvent(fakeRayBullet, 'onHitEnemy', fakeTicker, { target: obstacle });
              
              // Создаем эффект попадания во врага
              if (weaponConfig.raycastAnimation === 'impact') {
                createImpactPoint(currentX, currentY, 'hit', app, impactEffects);
              }
              ...
              if (penetrationLeft <= 0 && weaponConfig.ricochetEnemies && ricochetsLeft > 0) {
                ...
                const fakeRayBullet = { x: currentX, y: currentY };
                const fakeTicker = { elapsedMS: 0 };
                localTriggerBulletEvent(fakeRayBullet, 'onRicochet', fakeTicker, { target: obstacle });
      ...
      const fakeRayBullet = { x: currentX, y: currentY };
      const fakeTicker = { elapsedMS: 0 };
      localTriggerBulletEvent(fakeRayBullet, 'onExpire', fakeTicker);
      
      // Создаем визуальный эффект для всего луча или точки окончания
      if (weaponConfig.raycastAnimation === 'laser') {
        createRayVisual(startX, startY, currentX, currentY, app, rayEffects);
      } else if (weaponConfig.raycastAnimation === 'impact') {
        createImpactPoint(currentX, currentY, 'end', app, impactEffects);
      }
    }
    ...
    function createBullet() {
      ...
      for (let i = 0; i < weaponConfig.bulletsPerShot; i++) {
        ...
        const vx = Math.cos(finalAngle) * weaponConfig.bulletSpeed;
        const vy = Math.sin(finalAngle) * weaponConfig.bulletSpeed;
        
        // Вычисляем индивидуальную дальность с разбросом
        const bulletMaxRange = calculateRangeWithSpread(weaponConfig);

        // Определяем позицию появления: у игрока или у курсора
        const spawnX = weaponConfig.spawnAtCursor ? mousePosition.x : graphics.x;
        const spawnY = weaponConfig.spawnAtCursor ? mousePosition.y : graphics.y;
        
        // Создаем снаряд через хелпер
        const bullet = createBulletHelper(spawnX, spawnY, vx, vy, weaponConfig, bulletMaxRange);
    ...
    app.ticker.add((ticker) => {
      // --- Логика перезарядки ---
      const ammoState = { currentAmmo, isReloading };
      if (updateReloadSystem(ammoState, weaponConfig, Date.now(), reloadStartTime)) {
        currentAmmo = ammoState.currentAmmo;
        isReloading = ammoState.isReloading;
      }
      
      // --- Логика игрока ---
      updatePlayerRotation(graphics, mousePosition, rotationSpeed);
      updatePlayerMovement(graphics, keys, speed, ticker.deltaTime);
      ...
      for (let i = bullets.length - 1; i >= 0; i--) {
        ...
        localTriggerBulletEvent(b, 'onFlight', ticker);
        
        // --- Обновляем физические системы ---
        updateHomingSystem(b, mousePosition, weaponConfig, ticker);
        updateGravitySystem(b, weaponConfig, ticker);
        updateBulletMovement(b, ticker);

        let shouldRemove = shouldRemoveBullet(b);
        
        // Проверяем касание края экрана для событий (ДО рикошета!)
        if (isAtScreenEdge(b, app.screen, 4) && !b.hasTriggeredScreenEdge) {
          localTriggerBulletEvent(b, 'onScreenEdge', ticker);
        ...
        if (weaponConfig.ricochetWalls && b.ricochetsLeft > 0) {
          const ricochetResult = calculateWallRicochet(b, app.screen, 4);
          
          if (ricochetResult.hasRicocheted) {
            // --- Событие: onRicochet (при рикошете от стен) ---
            localTriggerBulletEvent(b, 'onRicochet', ticker);
        ...
        if (checkScreenBounds(b, app.screen, weaponConfig)) {
          shouldRemove = true;
        }
        ...
        for (const obstacle of obstacles) {
          if (obstacle.isAlive && checkBulletObstacleCollision(b, obstacle)) {
            ...
            if (b.penetrationLeft > 0) {
              ...
              dealDamage(obstacle, b.damage, respawn);
              
              // --- Событие: onHitEnemy (при попадании во врага) ---
              localTriggerBulletEvent(b, 'onHitEnemy', ticker, { target: obstacle });
              ...
              if (b.penetrationLeft <= 0) {
                if (weaponConfig.ricochetEnemies && b.ricochetsLeft > 0) {
                  ...
                  localTriggerBulletEvent(b, 'onRicochet', ticker, { target: obstacle });
        ...
        if (shouldRemove) {
          ...
          localTriggerBulletEvent(b, 'onExpire', ticker);
      ...
      constrainPlayerToBounds(graphics, app.screen);
